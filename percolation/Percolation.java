...
public class Percolation {
    ...
    public void open(int row, int col) {
        ...
        int shiftRow = row - 1;
        int shiftCol = col - 1;
        int flatIndex = xyTo1D(row, col) - 1;
        ...
        if (isValid(row, col - 1) && isOpen(row, col - 1)) {
            wqfGrid.union(flatIndex, xyTo1D(row, col - 1) - 1);
            wqfFull.union(flatIndex, xyTo1D(row, col - 1) - 1);
        }
        ...
        if (isValid(row - 1, col) && isOpen(row - 1, col)) {
            wqfGrid.union(flatIndex, xyTo1D(row - 1, col) - 1);
            wqfFull.union(flatIndex, xyTo1D(row - 1, col) - 1);
        }
